...
public class class_5087 extends ChunkGenerator {
    public static final MapCodec<class_5087> CODEC = RecordCodecBuilder.mapCodec(instance ->
            instance.group(
                    BiomeSource.CODEC.fieldOf("biome_source").forGetter(generator -> generator.biomeSource)
            ).apply(instance, class_5087::new)
    );

    public class_5087(BiomeSource biomeSource) {
        super(biomeSource);
    }
    ...
    public CompletableFuture<Chunk> populateNoise(Blender blender, NoiseConfig noiseConfig, StructureAccessor structureAccessor, Chunk chunk) {
        ...
        if (chunkPos.x == 0 && chunkPos.z == 0) {
            ...
            chunk.setBlockState(mutable.set(0, 0, 0), Blocks.DIAMOND_BLOCK.getDefaultState(), 2);

            for (int i = 1; i <= 4; i++) {
                chunk.setBlockState(mutable.set(0, i, 0), blockState, 2);
                chunk.setBlockState(mutable.set(i, 0, 0), blockState2, 2);
                chunk.setBlockState(mutable.set(0, 0, i), blockState3, 2);
            }
        }
