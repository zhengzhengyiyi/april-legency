...
public enum FoodRestriction implements StringIdentifiable {
    ...
    AIR("air_block", Items.AIR, "rule.food_restriction.maybe_edible");
    ...
    public static final Codec<FoodRestriction> CODEC = StringIdentifiable.createCodec(FoodRestriction::values);
    
    public static final Set<Item> RESTRICTABLE_ITEMS = Arrays.stream(values())
        .map(FoodRestriction::getItem)
        .filter(java.util.Objects::nonNull)
        .collect(Collectors.toSet());

    private FoodRestriction(String id, @Nullable Item item, String translationKey) {
        this.id = id;
        this.item = item;
