...
//                .requires(serverCommandSource -> serverCommandSource.getPermissions(2))
                .then(
    ...
    private static Text method_50081(UUID uUID, class_8376 arg) {
        String string = class_8376.field_43995.encodeStart(JsonOps.INSTANCE, arg).get().left().map(JsonElement::toString).orElse("Error! Oh Error!");
    ...
    private static int method_50071(ServerCommandSource serverCommandSource, Optional<RegistryEntry.Reference<Vote>> optional) {
        Random random = serverCommandSource.getWorld().random;
        UUID uUID = UUID.randomUUID();
        MinecraftServer minecraftServer = serverCommandSource.getServer();
        Set<Vote> set = minecraftServer.method_51115().method_50570();
        class_8376.class_8379 lv = class_8376.class_8379.method_50547(random);
        
        Optional<class_8376> optional2 = optional.isPresent() 
            ? class_8376.method_50529(uUID, minecraftServer, lv, optional.get().value())
            : class_8376.method_50528(uUID, set, minecraftServer, lv);
    ...
    private static int method_50067(ServerCommandSource serverCommandSource, class_8290 arg) {
        RegistryEntry.Reference<Vote> reference = class_8293.method_50208(serverCommandSource.getWorld().getRandom());
        return method_50068(serverCommandSource, arg, reference);
    }

    private static int method_50068(ServerCommandSource serverCommandSource, class_8290 arg, RegistryEntry.Reference<Vote> reference) {
        return (switch (arg) {
    ...
    private static int method_50070(ServerCommandSource serverCommandSource, RegistryEntry.Reference<Vote> reference, class_8290 arg, NbtElement nbtElement) {
    ...
    private static int method_50069(ServerCommandSource serverCommandSource, RegistryEntry.Reference<Vote> reference) {
    ...
    private static void method_50084(MinecraftServer minecraftServer, PrintStream printStream, Random random, RegistryEntry.Reference<Vote> reference) {
    ...
    private static int method_50091(ServerCommandSource serverCommandSource, boolean bl) {
        MinecraftServer minecraftServer = serverCommandSource.getServer();
        Registry<Vote> registry = serverCommandSource.getRegistryManager().getOrThrow(RegistryKeys.RULES);
